...
const KNOWN_SKILLS = [
  'update-tests', 'update-uat', 'regression', 'sync-design', 'done-check', 'update-observability',
...
const DOC_SPECS: DocSpec[] = [
  ...
  { expectedName: 'observability-expectations.md', label: 'Observability Expectations', icon: 'pulse',
    equivalents: ['observability.md', 'logging-standards.md', 'docs/observability.md', 'monitoring.md'] },
...
function detectAiTools(root: string): AiToolDetection[] {
  ...
  return tools.map(tool => {
    let detected = tool.indicators.some(indicator =>
      fs.existsSync(path.join(root, indicator))
    );

    // Guard against circular detection: if AGENTS.md was written by this extension,
    // do not treat it as evidence that Codex is in use.
    if (tool.id === 'codex' && detected) {
      const agentsPath = path.join(root, 'AGENTS.md');
      if (fs.existsSync(agentsPath)) {
        try {
          const content = fs.readFileSync(agentsPath, 'utf8');
          if (content.includes('auto-generated by Claude Code Workflow')) {
            detected = false;
          }
        } catch { /* ignore read errors — leave detected as-is */ }
      }
    }
